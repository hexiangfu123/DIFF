...
class GaussianDiffusion(nn.Module):
    ...
    def calculate_for_diffusion(self):
        # x_t = alpha_t^-2 * x_(t-1) + beta_t^-2 * noise_t
        # alpha_t + beta_t = 1
        # alpha_cumprod_t = alpha_0 * alpha_1 * ... * alpha_t
        # x_t = alpha_cumprod_t^-2 * x_0 + (1 - alpha_cumprod_t)^-2 * noise
        alphas = 1.0 - self.betas   # betas = 
        ...
        self.sqrt_one_minus_alphas_cumprod = th.sqrt(1.0 - self.alphas_cumprod) # beta_cumprod_t
        self.log_one_minus_alphas_cumprod = th.log(1.0 - self.alphas_cumprod) # 
        self.sqrt_recip_alphas_cumprod = th.sqrt(1.0 / self.alphas_cumprod) # 1/alpha_cumprod_t^-2
        self.sqrt_recipm1_alphas_cumprod = th.sqrt(1.0 / self.alphas_cumprod - 1) # beta_cumprod_t^-2 / alpha_cumprod_t^-2
    ...
    def p_sample(self, model, x_start, item_emb, user_emb, mask, steps, sampling_noise=False):
        assert steps <= self.steps, "Too much steps in inference."
        
        # x_t = th.randn(item_emb.shape[0], model.args.score_dim).to(item_emb.device)
        x_t = x_start
        ...
        #         x_t = model(user_emb, item_emb, x_t, t, mask)
        #     return x_t
        ...
        for i in indices:
            ...
            t = th.tensor([i] * x_t.shape[0]).to(x_t.device)
            # ipdb.set_trace()
            out = self.p_mean_variance(model, x_t, t, item_emb, user_emb, mask)
            if sampling_noise:
                noise = th.randn_like(x_t)
                nonzero_mask = (
                    (t != 0).float().view(-1, *([1] * (len(x_t.shape) - 1))) # t!=0 [400, 1] 1 # t==0 [400, 1] 0
                )  # no noise when t == 0
                x_t = out["mean"] + nonzero_mask * th.exp(0.5 * out["log_variance"]) * noise # [400, 300]
                # x_t = avg + exp^(1/2*var)*noise
            else:
                x_t = out["mean"]
        return x_t
    
    def training_losses(self, model, x_start, item_emb, user_emb, mask, reweight=False):
        # ipdb.set_trace()
        batch_size, device = x_start.size(0), x_start.device
        ts, pt = self.sample_timesteps(batch_size, device, 'uniform') # ts 对step(t)随机采样
        noise = th.randn_like(x_start)  # 随机噪声 batch_size * output_dim
        if self.noise_scale != 0.:
        ...
        model_output = model(user_emb, item_emb, x_t, ts, mask)
        # ipdb.set_trace()
        # model_output = th.ones_like(x_t)
        target = x_start
        ...
        #         weight = self.SNR(ts - 1) - self.SNR(ts) # [batch_size] 
        #         # alpha_cumprod_(t-1)/(1-alpha_cumprod_(t-1)) - alpha_cumprod_t/(1-alpha_cumprod_t)
    ...
    def p_mean_variance(self, model, x, t, item_emb, user_emb, mask):
        """
        ...
        model_output = model(user_emb, item_emb, x, t, mask)
