...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, model, x_start, user_emb, mask, steps, sampling_noise=False):
        ...
        for i in indices:
            ...
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            # ipdb.set_trace()
            out = self.p_mean_variance(model, x_t, t, user_emb, mask, x_start)
            if sampling_noise:
    ...
    def training_losses(self, model, x_start, user_emb, mask, reweight=False):
        # ipdb.set_trace()
        ...
        terms = {}
        if model.args.split_diff_score:
            model_output = model(x_t, ts)
        else:
            model_output = model(user_emb, x_t, None, ts, mask)
        target = {
    ...
    def p_mean_variance(self, model, x, t, user_emb, mask, item_emb):
        """
        ...
        if model.args.split_diff_score:
            ...
            model_output = model(user_emb, x, None, t, mask)
